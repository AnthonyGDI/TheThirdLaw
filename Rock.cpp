...
class CRockRegistrator
	: public IEntityRegistrator
{
	virtual void Register() override
	{
		CGameFactory::RegisterGameObject<CRock>("Rock");
...
CRockRegistrator g_bulletRegistrator;

void CRock::UpdateEntity(std::vector<IEntity*> entitiesInRange)
{
	//Affect all entities in range
	for (int i = 0; i < entitiesInRange.size(); i++)
	{
		auto *pGameObject = gEnv->pGame->GetIGameFramework()->GetGameObject(entitiesInRange[i]->GetId());
	...
	CryLog("Updated Rock. Weights: %f, %f, %f, %f, %f, %f", propertyWeights[0], propertyWeights[1], propertyWeights[2], propertyWeights[3], propertyWeights[4], propertyWeights[5]);
}

void CRock::AffectEntity()
{
	CryLog("I was affected.");
}

void CRock::PostInit(IGameObject *pGameObject)
{
